...
def find_jobs():
    ...
    DatabaseConnector.get_instance().add_timeline_evidence_event(
        current_user.id, job_name, job_deadline
    )
    resp = RchilliConnector.get_instance().job_search(
        RchilliConnector.get_instance().job_autocomplete(job_name)
    )["Skills"]
...
def parse_certificate():
    ...
    DatabaseConnector.get_instance().add_certificate_event(
        current_user.id, resp["courseName"], resp["date"], resp["url"], resp["userName"]
    )
    return resp
