...
def favicon():
    ...
    return send_from_directory(
        os.path.join(core_route.root_path, "static/image"),
        "favicon.ico",
    ...
    if request.method == "POST":
        if avatar is not None:
            if avatar_name[-3:] in ["jpg", "png"]:
                ...
                avatar.save(
                    os.path.join(current_app.config["UPLOAD_IMAGE_FOLDER"], file_name)
                )

                Analytics.get_instance().increment_image_count()
                DatabaseConnector.get_instance().update_record(
                    "id", current_user.id, "avatar", file_name
                )
...
def index():
    ...
    return render_template(
        "/ru/index_ru.html", title="Digital Professional Me", userName=current_user.name
    )
    ...
    if request.method == "POST":
        try:
            ...
            if cv_link != "":
                try:
                    file_name = (
                        f"hhCv_{Analytics.get_instance().summary_cv_count()}.pdf"
                    )
            ...
            cur_user = DatabaseConnector.get_instance().find_record(
                "id", current_user.id
            )

            if cur_user is not None:
                rchilli_data = RchilliConnector.get_instance().rchilli_parse(file_name)
                json_data, skills_array = ServiceContainer.get_instance().json_convert(
                    rchilli_data
                )
                timeline_events = ServiceContainer.get_instance().timeline_parse(
                    rchilli_data
                )
                ...
                DatabaseConnector.get_instance().update_record(
                    "id", current_user.id, "jsondata", json_data
                )
                DatabaseConnector.get_instance().update_record(
                    "id", current_user.id, "rchillidata", rchilli_data
                )
                DatabaseConnector.get_instance().update_record(
                    "id", current_user.id, "timelineEvents", timeline_events
                )
                return "200"
        except Exception as e:
            print(e)
