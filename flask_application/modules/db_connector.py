...
class DatabaseConnector:
    ...
    def __init__(self):
        if not DatabaseConnector.__instance:
            self.client = MongoClient(
                f'mongodb://root:example@{current_app.config["MONGO_MODE"]}', 27017
            )
    ...
    def update_record(self, findKey, findValue, key, value):
        self.collection_users.find_one_and_update(
            {findKey: findValue}, {"$set": {key: value}}
        )
    ...
    def update_recommendation_clicks(self, user_id):
        count = self.collection_users.find_one({"id": user_id})[
            "recommendationClickCounter"
        ]
        self.collection_users.find_one_and_update(
            {"id": user_id}, {"$set": {"recommendationClickCounter": count + 1}}
        )
